...
function deterministicGenerateK(curve, hash, d, checkSig, nonce) {
  ...
  k = HmacSHA256(Buffer.concat([v, new Buffer([0]), x, hash]), k); // Step E

  v = HmacSHA256(v, k); // Step F

  k = HmacSHA256(Buffer.concat([v, new Buffer([1]), x, hash]), k); // Step G

  v = HmacSHA256(v, k); // Step H1/H2a, ignored as tlen === qlen (256 bit)
  // Step H2b
  ...
  while (T.signum() <= 0 || T.compareTo(curve.n) >= 0 || !checkSig(T)) {
    ...
    v = HmacSHA256(v, k); // Step H1/H2a, again, ignored as tlen === qlen (256 bit)
    // Step H2b again
