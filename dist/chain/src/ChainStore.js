...
var _tuscjsWs = require("tuscjs-ws");
...
function () {
  ...
  _proto.init = function init(subscribe_to_new) {
    ...
    var _init = function _init(resolve, reject) {
      ...
      var db_api = _tuscjsWs.Apis.instance().db_api();
        ...
        for (var i = 0; i < optional_objects.length; i++) {
          ...
          if (optional_object) {
            ...
            */
            ...
            if (delta < 60) {
              _tuscjsWs.Apis.instance().db_api().exec("set_subscribe_callback", [_this.onUpdate.bind(_this), subscribe_to_new]).then(function () {
  ...
  _proto.getAsset = function getAsset(id_or_symbol) {
    ...
    _tuscjsWs.Apis.instance().db_api().exec("lookup_asset_symbols", [[id_or_symbol]]).then(function (asset_objects) {
  ...
  _proto.getAccountRefsOfKey = function getAccountRefsOfKey(key) {
    ...
    if (this.get_account_refs_of_keys_calls.has(key)) return this.account_ids_by_key.get(key);else {
      ...
      _tuscjsWs.Apis.instance().db_api().exec("get_key_references", [[key]]).then(function (vec_account_id) {
  ...
  _proto.getAccountRefsOfAccount = function getAccountRefsOfAccount(account_id) {
    ...
    if (this.get_account_refs_of_accounts_calls.has(account_id)) return this.account_ids_by_account.get(account_id);else {
      ...
      _tuscjsWs.Apis.instance().db_api().exec("get_account_references", [account_id]).then(function (vec_account_id) {
        var refs = _immutable["default"].Set();
  ...
  _proto.getBalanceObjects = function getBalanceObjects(address) {
    ...
    if (current === undefined) {
      ...
      _tuscjsWs.Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(function (balance_objects) {
        var set = new Set();
  ...
  _proto.fetchObject = function fetchObject(id, force, autosubscribe, no_full_account) {
    ...
    if (result === undefined) {
      ...
      if (!_tuscjsWs.Apis.instance().db_api()) return null;

      _tuscjsWs.Apis.instance().db_api().exec("get_objects", [[id]]).then(function (optional_objects) {
        //if(DEBUG) console.log("... optional_objects",optional_objects ? optional_objects[0].id : null)
  ...
  _proto.fetchAllWorkers = function fetchAllWorkers() {
    ...
    return new Promise(function (resolve, reject) {
      _tuscjsWs.Apis.instance().db_api().exec("get_all_workers", []).then(function (workers_array) {
  ...
  _proto.fetchWitnessByAccount = function fetchWitnessByAccount(account_id) {
    ...
    return new Promise(function (resolve, reject) {
      _tuscjsWs.Apis.instance().db_api().exec("get_witness_by_account", [account_id]).then(function (optional_witness_object) {
  ...
  _proto.fetchCommitteeMemberByAccount = function fetchCommitteeMemberByAccount(account_id) {
    ...
    return new Promise(function (resolve, reject) {
      _tuscjsWs.Apis.instance().db_api().exec("get_committee_member_by_account", [account_id]).then(function (optional_committee_object) {
  ...
  _proto.fetchFullAccount = function fetchFullAccount(name_or_id, autosubscribe) {
    ...
    if (subChanged || !this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
      ...
      _tuscjsWs.Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], autosubscribe]).then(function (results) {
        if (results.length === 0) {
          if (is_object_id) {
        ...
        if (sub_to_objects.length) _tuscjsWs.Apis.instance().db_api().exec("get_objects", [sub_to_objects]);
  ...
  _proto.fetchRecentHistory = function fetchRecentHistory(account, limit) {
    ...
    pending_request.promise = new Promise(function (resolve, reject) {
      _tuscjsWs.Apis.instance().history_api().exec("get_account_history", [account_id, most_recent, limit, start]).then(function (operations) {
      ...
      case "call_order":
        ...
        if (call_account && call_account !== true) {
          ...
          if (!call_orders.has(object.id)) {
            ...
            _tuscjsWs.Apis.instance().db_api().exec("get_objects", [[object.id]]); // Force subscription to the object in the witness node by calling get_objects
  ...
  _proto.getObjectsByVoteIds = function getObjectsByVoteIds(vote_ids) {
    ...
    if (missing.length) {
      ...
      _tuscjsWs.Apis.instance().db_api().exec("lookup_vote_ids", [missing]).then(function (vote_obj_array) {
