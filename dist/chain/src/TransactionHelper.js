...
helper.unique_nonce_uint64 = function () {
  var entropy = helper.unique_nonce_entropy = function () {
    if (helper.unique_nonce_entropy === null) {
      ...
      return parseInt(_secureRandom["default"].randomUint8Array(1)[0]);
    } else {
      return ++helper.unique_nonce_entropy % 256;
    }
  }();

  var _long = _bytebuffer.Long.fromNumber(Date.now()); //console.log('unique_nonce_uint64 date\t',ByteBuffer.allocate(8).writeUint64(long).toHex(0))
...
helper.to_json = function (tr, broadcast) {
  if (broadcast === void 0) {
    broadcast = false;
  }
  ...
  return function (tr, broadcast) {
    ...
    if (broadcast) {
      var net = _tuscjsWs.Apis.instance().network_api();
